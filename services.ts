...
export const SQL_UPDATE_USER_PASSWORD_FIX = `-- ฟังก์ชันสำหรับ Admin เปลี่ยนรหัสผ่านให้ User อื่น
...
export const updateUserStatus = async (userId: string, status: 'online' | 'offline') => {
    // Also update last_active to now
...
export const runConnectionTest = async (onProgress: (result: ConnectionTestResult) => void) => {
    ...
    const tests = [
        {
            name: "0. ⚡ One-Click Setup (Start Here)",
            requiresAuth: false,
            action: async () => {
                 return { 
                    success: true, 
                    details: "รัน Script นี้เพื่อติดตั้งระบบทั้งหมดและสร้าง Admin", 
        ...
        {
             name: "12. ตรวจสอบคอลัมน์ Last Active (Online Status)",
             requiresAuth: false,
             action: async (currentUserId?: string) => {
                  const { error } = await supabase.from('profiles').select('last_active').limit(1);
                  return {
                      success: !error,
                      details: error ? "ไม่พบคอลัมน์ 'last_active' (ต้องรัน SQL Fix)" : "พบคอลัมน์ 'last_active'",
                      fix: error ? SQL_ADD_LAST_ACTIVE : undefined
                  }
             }
        }
    ];
